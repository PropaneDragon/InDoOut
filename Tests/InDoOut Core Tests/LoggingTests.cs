...
namespace InDoOut_Core_Tests
{
    ...
    public class LoggingTests
    {
        ...
        public void AddToLog()
        {
            ...
            Assert.AreEqual($"[{log.Logs[0].Time.ToString("yyyy-MM-dd HH:mm:ss")}][{log.Logs[0].CallingAssembly.GetName().Name}][!] This is an error", log.Logs[0].ToString());
            ...
            Assert.AreEqual($"[{log.Logs[1].Time.ToString("yyyy-MM-dd HH:mm:ss")}][{log.Logs[1].CallingAssembly.GetName().Name}][@] This is a warning", log.Logs[1].ToString());
            ...
            Assert.AreEqual($"[{log.Logs[2].Time.ToString("yyyy-MM-dd HH:mm:ss")}][{log.Logs[2].CallingAssembly.GetName().Name}][i] This is a message", log.Logs[2].ToString());
            ...
            Assert.AreEqual(2, log.Logs.Count);
            Assert.AreEqual($"[{log.Logs[1].Time.ToString("yyyy-MM-dd HH:mm:ss")}][{log.Logs[1].CallingAssembly.GetName().Name}][i] This is another message", log.Logs[1].ToString());
            Assert.AreEqual(Assembly.GetExecutingAssembly(), log.Logs[1].CallingAssembly);
            Assert.AreEqual(LogMessage.LogLevel.Info, log.Logs[1].Level);
            Assert.IsTrue((DateTime.Now - log.Logs[1].Time).TotalSeconds < 1d);

            Assert.AreEqual($"[{log.Logs[0].Time.ToString("yyyy-MM-dd HH:mm:ss")}][{log.Logs[0].CallingAssembly.GetName().Name}][i] This is a message", log.Logs[0].ToString());
            Assert.AreEqual(Assembly.GetExecutingAssembly(), log.Logs[0].CallingAssembly);
